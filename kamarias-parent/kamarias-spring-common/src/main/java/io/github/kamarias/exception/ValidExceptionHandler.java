...
import io.github.kamarias.web.KamariasWebMvcAutoConfiguration;
...
@ConditionalOnMissingBean(ValidExceptionHandler.class)
...
public class ValidExceptionHandler {
    ...
    public AjaxResult<Object> handleValidateException(BindException e) {
        ...
        return AjaxResult.error("参数校验异常", errorMap);
    }
    ...
    public AjaxResult<Object> handleValidateException(MethodArgumentNotValidException e) {
        BindingResult bindingResult = e.getBindingResult();
        Map<String, String> errorMap = new HashMap<>(8);
        bindingResult.getFieldErrors().forEach(item -> errorMap.put(item.getField(), item.getDefaultMessage()));
        return AjaxResult.error("参数校验异常", errorMap);
    ...
    public AjaxResult<Object> handleValidException(ConstraintViolationException e) {
        // 获取异常信息
